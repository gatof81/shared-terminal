...
3. **Notify every user** to re-enter their PATs, SSH keys, and `secret` env vars on each affected session. There is no audit trail of which values were stranded — the ciphertext doesn't preserve the original variable name in plaintext, so users have to reconstruct the list themselves.
