...
- **D1 is the source of truth** for session metadata (`session_id`, `container_id`, `status`, `env_vars`, `cols/rows`, and the opaque `external_ref` (#418) — client data ≤128 chars, set on create or via `PATCH /api/sessions/:id`, exact-match filterable with `?externalRef=` on both list endpoints) and for the `session_configs` row that holds typed env entries (with `secret`-typed values AES-GCM-encrypted at rest), repo + auth config, lifecycle hook commands, dotfiles + agent-seed config, and resource caps. Config is bound at create time — there is no general "edit config of a running session" flow. **Exceptions:** (#270) admins can live-edit `cpu_limit` / `mem_limit` via `PATCH /admin/sessions/:id/resources` (persist to `session_configs` AND `docker update` the running container); (#190) owners can live-edit `config.ports[]` via `PATCH /api/sessions/:id/ports` — pure metadata (no `docker` call) since the dispatcher proxies the container by name over the shared network, see "Port-exposure dispatcher" below.
...
- **`writeEnvFile` (#277) is an exception to at-rest encryption.** When a session opts into `config.writeEnvFile`, the bootstrap stage writes decrypted `secret` values as cleartext into `<WORKSPACE_ROOT>/<sessionId>/.env` on the host bind mount (so `secrets.ts`'s "D1 export yields only ciphertext" guarantee no longer holds for them), and because soft delete preserves the workspace dir, that `.env` survives a soft delete — only `?hard=true` purges it. Operator-facing writeup in [docs/SECURITY.md → Secrets at rest](./docs/SECURITY.md#secrets-at-rest); rationale for not shredding on soft delete is in the `writeEnvFile.ts` header.

### Bootstrap (lifecycle hooks)

`POST /api/sessions` returns 201 immediately and runs the bootstrap pipeline asynchronously: git identity → repo clone → dotfiles → agent seed → `postCreate` command. The pipeline first waits (≤60 s, best-effort) on the **runtime-readiness sentinel** (#393) — `container.start()` returns before the entrypoint finishes, and the entrypoint's `~/.npm-global` symlink-swap leaves a window where exec'd image binaries (`claude`) don't resolve; the entrypoint touches `/tmp/.st-ready` as its last step (`RUNTIME_READY_SENTINEL` in `dockerManager.ts`), `DockerManager.isRuntimeReady` probes it on demand with a positive per-container cache, and `GET /api/sessions/:id` surfaces it as `runtimeReady: boolean | null` (null = not running / probe error; false forever on pre-sentinel containers — recycle). API clients should gate their first exec on it — contract table in [docs/EXEC_API.md → Runtime readiness](./docs/EXEC_API.md#runtime-readiness-393). The clone-before-dotfiles ordering is load-bearing: dotfiles may reference repo-specific config, and the agent seed is intentionally last so a cloned project's `CLAUDE.md` is on disk before the seed fires. `postCreate` is gated by an atomic `session_configs.bootstrapped_at` flip so it runs **exactly once** per session even under concurrent retries (see `bootstrap.ts`). `postStart` re-runs on every container start (intended for daemons). Total wall-clock cap is 10 minutes — `streamExec` is destroyed past that and the session hard-fails. Graceful shutdown (SIGTERM/SIGINT) aborts in-flight bootstraps and awaits their teardown, landing interrupted sessions at `status=failed` with the reason in the persisted log (#348); a SIGKILL still strands them at `running` with `bootstrapped_at` NULL — accepted gap, documented on `drainInFlightBootstraps`. Live output streams over `/ws/bootstrap/<sessionId>` (auth identical to terminal attach but routed to `BootstrapBroadcaster`'s per-session listener set, not `docker.attach`).
...
`backup.ts` — host-shell tools (`npm run backup` / `npm run restore -- <dir> [--force]` inside the backend container), deliberately not API surface. Backup: one deterministic `<table>.jsonl` per table (FK-dependency order in the `TABLES` array — restore replays it top to bottom), `workspaces/<sid>.tar.gz` (workspace AND uploads dir, gzip not zstd — already in the image), `manifest.json` written LAST so restore can refuse interrupted runs. Restore: `migrateDb()` first (fresh target gets schema; backup columns older than current schema stay NULL via explicit-column INSERTs), refuses non-empty targets without `--force` (`INSERT OR REPLACE` makes forced re-runs converge), **refuses a `SECRETS_ENCRYPTION_KEY` that can't decrypt a sampled blob with NO force override** (one decrypt round-trip before any write), entry-allowlists tarballs before extraction (path traversal), and lands previously-`running` rows as `stopped`/NULL-container (what reconcile would conclude). Operator recipe in [docs/DEPLOYMENT.md → Backup & restore](./docs/DEPLOYMENT.md#backup--restore-240).

### Per-user quotas (#202)

`userQuotas.ts`. Three layers: deployment defaults (`MAX_ACTIVE_SESSIONS_PER_USER` for the count; `USER_MAX_TOTAL_CPU` cores / `USER_MAX_TOTAL_MEM` MiB env vars for the budgets, **unset = unlimited** so upgrades are behaviour-neutral), per-user override columns on `users` (migration v12, NULL = default, units = nano-CPUs/bytes matching `session_configs`), and create-time enforcement in `POST /sessions` only (existing sessions never retroactively affected; `/start` deliberately ungated per the issue). (#420) admins can pass `ownerUserId` on `POST /sessions` to create on behalf of another user — the session is owned by the target, quotas resolve and charge against the target (same 429 shapes), non-admins get 403, and an owner≠caller create emits an audit log line. The session-COUNT cap stays atomic inside `sessions.create`'s INSERT guard (the route just resolves the per-user number to fold in); the CPU/RAM budget check is read-then-insert and racy under same-user concurrent creates — accepted v1 gap, documented in the module header. Budget math counts EFFECTIVE allocations (same `Math.min(stored ?? default, operator cap)` as spawn) so a stale 8-core stored cap on a 4-core-max deployment burns 4. Admin surface: `GET /admin/users` (overrides + effective + usage, 3 D1 reads total) and `PATCH /admin/users/:id/quotas` (`null` clears an override; budget bounds deliberately exceed the per-session max — totals span N sessions).

### HTTP exec API (#381)

`routes/exec.ts` exposes `streamExec`/`killExecProcessGroup` over HTTP for external automation (the agenthub seam): `POST /api/sessions/:id/exec` streams NDJSON events (`started` → `output` → `exit`|`error`), `GET /:execId` is the recovery surface, `POST /:execId/kill` maps 1:1 to `killExecProcessGroup`. Canonical contract in [docs/EXEC_API.md](./docs/EXEC_API.md). Key invariants: `started` is strictly the first event (early stderr that beats the pgid sentinel is buffered, bounded); exit `reason` is attributed via a first-writer-wins kill-intent mark in `execRegistry.ts` (`killed`/`timeout` walk back to `exited` when the kill outcome is `already-exited`); kill on a registry-exited exec answers WITHOUT re-probing /proc (pgid recycling hazard); client disconnect does NOT kill the process but resumes a backpressure-paused stream so the exec can drain. The registry is process-local and in-memory — restart loses it and the status endpoint answers `state:"unknown"` for any id it doesn't hold (deliberately not 404). These routes are the first to emit `X-Request-Id` publicly; the global-emission decision stays #376's. Limits: 4 concurrent execs/session, 1 h wall-clock cap (also the default), exec env rides `validateEnvVars`, `execIp` limiter covers start + kill but deliberately not status reads.

### Port-exposure dispatcher (#190)

Sessions can declare ports under `config.ports[]`. Rather than publishing host ports, every session container joins a shared user-defined Docker network (`SESSIONS_NETWORK`, default `sessions-net`, set as `HostConfig.NetworkMode` in `DockerManager.spawn()`); the backend `app` service joins the same network in `docker-compose.yml`. The dispatcher reaches a session by its Docker `--name` (`st-<sid[:12]>`) over Docker's embedded DNS, so **no `-p` host publishing happens**. `sessions_port_mappings` (`{ session_id, container_port, host_port, is_public }`) is now a denormalised copy of the declarative `config.ports[]` joined against the live session row — written from config via `mappingsFromConfig` so the dispatcher's hot path is one indexed point read instead of a JSON parse. The `host_port` column is **vestigial** (kept `NOT NULL`, written = the container port; dropping it is a deferred table-rebuild ticket). `allowPrivilegedPorts: true` re-grants exactly `CAP_NET_BIND_SERVICE` (and only that capability) on `docker run`. The mapping table is rewritten on every container start (spawn, startContainer Case-2, reconcile) and by `PATCH /api/sessions/:id/ports`, and cleared on stop / kill / hard-delete via direct `clearPortMappings(sessionId)` calls in `dockerManager.ts`.

Because the exposed set is pure metadata (no Docker `PortBindings` to recreate), owners live-edit it via `PATCH /api/sessions/:id/ports` (owner-gated, mirrors `PATCH /sessions/:id/env`): persist `ports_json` (`updatePorts`) then rewrite the mappings (`setPortMappings(mappingsFromConfig(ports))`) — no `docker` call, applied immediately on a running session. `GET /api/sessions/:id/ports` returns the declared set + `allowPrivilegedPorts` for the toolbar editor. A newly-added privileged port (`< 1024`) is rejected unless the session was *created* with `allowPrivilegedPorts` (the cap is fixed at spawn). **Deployment note:** the `sessions-net` network must be created once (`docker network create sessions-net`) before `docker compose up`, and sessions running across the upgrade must be stopped+started once to re-join it (see [docs/DEPLOYMENT.md](./docs/DEPLOYMENT.md)). Two known follow-ups: the live proxy path is not yet verified end-to-end against a real Docker daemon ([#290](https://github.com/gatof81/shared-terminal/issues/290)), and the shared `sessions-net` makes session containers mutually reachable by name — an accepted single-tenant tradeoff, with multi-tenant isolation tracked in [#291](https://github.com/gatof81/shared-terminal/issues/291).

`portDispatcher.ts` is an Express middleware + WS-upgrade handler that intercepts requests whose Host header parses as `p<port>-<sessionId>.<PORT_PROXY_BASE_DOMAIN>`, looks up the runtime mapping via `lookupDispatchTarget` (single JOIN of `sessions_port_mappings` ⋈ `sessions WHERE status='running'`, returning `container_name` + `is_public` + owner), and reverse-proxies to `http://<container_name>:<containerPort>` via `http-proxy`. Auth gate: `public: false` requires the same `st_token` cookie + ownership the rest of the app uses; `public: true` skips auth (webhook / OAuth-callback shape). Both HTTP and WS paths run an `isAllowedWsOrigin` CSWSH/CSRF check before `authorize()` so a cross-origin browser request from `evil.com` can't ride the user's `SameSite=None` cookie. Because that check *allows a missing `Origin`* (legit non-browser callers), private ports additionally reject `Sec-Fetch-Site: cross-site` inside `authorize()` (#302) — closing the GET-CSRF gap where a cross-site `<img>`/`<script>`/link omits `Origin` but the cookie still auto-attaches; `same-origin`/`same-site`/`none`/absent all pass, and public ports skip the gate (cross-site webhook/OAuth shape). The dispatcher mounts BEFORE `express.json` / `cookieParser` / CORS so the proxied body stream isn't consumed and CORS isn't overlaid on the proxied app's response.
...
- `sessions.assertOwnership(sessionId, userId)` is the choke point for owner-only authorization on REST and WebSocket paths. Two graded siblings widen it: `assertCanObserve` (owner / admin / group-lead — **read-only**, `assertCanObserve`'s docblock is explicit that no write is granted) gates observe-mode attach and read surfaces (tab list, ports read, observe-log read); `assertCanOperate` (owner / admin — **write**, deliberately NO lead arm) gates the write surfaces an admin can drive on a foreign session: the non-observe WS attach (full terminal), the exec API (start/status/kill, #416), `POST /sessions/:id/start` (the session-lifecycle start verb, #429 — so Agent Hub's admin identity can auto-start a specialist session on the owner's behalf; **stop/delete stay owner-driven** — the owner and the idle sweeper own teardown), `PATCH /env`, `PATCH /ports`, `PATCH /sessions/:id` (`externalRef`, #418), and tab create/delete/search. Those REST routes additionally set `res.locals.skipIdleBump` when the caller isn't the owner, so an admin acting on a foreign session doesn't reset the owner's idle-auto-stop clock (the #300 principle); the `/ports` *read* uses the observe tier (`assertCanObserve`) since viewing is owner/admin/lead. Keeping observe and operate as separate primitives is load-bearing — it stops a future "widen observe" change from silently widening the write surface. A cross-user WS attach writes a `session_observe_log` row tagged `mode='observe'|'operate'` (migration v13) so the admin dashboard and the owner's own per-session log distinguish "admin watched" from "admin drove"; a cross-user exec writes the same `mode='operate'` row spanning the exec's start→exit (#416); a cross-user session start writes a point-in-time `mode='operate'` row (#429), fail-closed — a D1 failure on that audit INSERT aborts the start before any container spawns, since the audit is the mitigation for widening admin power by that verb. Resource caps are the other admin-editable axis (`PATCH /admin/sessions/:id/resources`); there is no edit-running-config surface, so "config" is not admin-operable.
...
- `PATCH /api/admin/sessions/:id/resources` (#270) — live-edit `cpu_limit` / `mem_limit`. Body `{ cpuLimit?, memLimit? }` validated against the same EFFECTIVE_*_MIN/MAX bounds the create-time schema uses (operator cap can only LOWER); at least one field required. Persists to `session_configs` FIRST, then `docker.updateResources` → `container.update({ NanoCpus, Memory, MemorySwap: Memory })` on the running container; stats cache is invalidated for that container so the next refresh re-samples against the new cgroup denominator. Persist-before-apply is the chosen rollback shape — if Docker rejects, the row is already updated so the next session start picks up the new caps anyway. Cgroup-v2 rejects a `Memory` drop below current usage with several substring shapes (`lower than current` / `less than memory reservation` / `memory limit too low` / `Out of memory`); the handler matches all of them and returns **409** with a "free memory first" hint. 204 on success.
...
Migrations are versioned and ledger-tracked (#349): `migrateDb()` applies pending entries from the append-only `MIGRATIONS` array in `db.ts` and records each in `schema_migrations`; steady-state boot cost is two D1 calls. Never renumber or edit a shipped migration — append. Versions 1-11 predate the ledger and must stay idempotent (pre-ledger deployments re-run them once against an existing schema).
