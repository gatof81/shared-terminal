...
- `sessions.assertOwnership(sessionId, userId)` is the choke point for owner-only authorization on REST and WebSocket paths. Two graded siblings widen it: `assertCanObserve` (owner / admin / group-lead — **read-only**, `assertCanObserve`'s docblock is explicit that no write is granted) gates observe-mode attach and read surfaces (tab list, ports read, observe-log read); `assertCanOperate` (owner / admin — **write**, deliberately NO lead arm) gates the write surfaces an admin can drive on a foreign session — the non-observe WS attach (full terminal) today; the REST write routes (`PATCH /env`, `PATCH /ports`, tab create/delete/search) move onto it in the REST follow-up. Keeping observe and operate as separate primitives is load-bearing — it stops a future "widen observe" change from silently widening the write surface. A cross-user WS attach writes a `session_observe_log` row tagged `mode='observe'|'operate'` (migration v13) so the admin dashboard and the owner's own per-session log distinguish "admin watched" from "admin drove". Resource caps are the other admin-editable axis (`PATCH /admin/sessions/:id/resources`); there is no edit-running-config surface, so "config" is not admin-operable.
